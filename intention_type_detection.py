        ...
        for seed in [128, 256, 512, 1024, 2048]:
            ...
            encoded_dataset = dataset.map(
                lambda example: preprocess_data(
                    examples=example,
                    tokenizer=tokenizer,
                    labels=LABELS_INTENTION,
                    padding=config["intention"]["tokenizer"]["padding"],
                    truncation=config["intention"]["tokenizer"]["truncation"],
                    max_length=config["intention"]["tokenizer"]["max_length"]
                ),
            ...
            bert_model = AutoModelForSequenceClassification.from_pretrained(
                ...
                num_labels=len(LABELS_INTENTION),
                id2label=id2label,
                label2id=label2id
            )
            ...
            trainer = Trainer(
                ...
                tokenizer=tokenizer,
                compute_metrics=compute_metrics_intention,
